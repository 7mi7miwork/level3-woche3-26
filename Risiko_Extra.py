...
║          Tkinter GUI Version mit Hilfe & Extras              ║
║          [FIX: Button-Style Konflikt & Log-Reset]            ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# ───────────────────────── HILFE-SYSTEM ──────────────────────────
HELP_MODES = {
    "easy": {"tooltips": True, "hints": True, "tutorial": True, "highlight_moves": True, "strategic_tips": True},
    "medium": {"tooltips": True, "hints": False, "tutorial": False, "highlight_moves": False, "strategic_tips": False},
    "hard": {"tooltips": False, "hints": False, "tutorial": False, "highlight_moves": False, "strategic_tips": False},
}

HELP_TEXTS = {
    "initial": {
        "title": "📍 STARTAUFSTELLUNG",
        "text": "Platziere deine Starttruppen auf deinen Gebieten.\n• Klicke auf ein eigenes Gebiet, um +1 Truppe hinzuzufügen.\n• Alle Spieler platzieren abwechselnd.",
        "easy_hint": "💡 Tipp: Verteile Truppen gleichmäßig auf Grenzgebiete – so bist du besser geschützt!"
    },
    "reinforcement": {
        "title": "➕ VERSTÄRKUNG",
        "text": "Erhalte Truppen basierend auf:\n• Anzahl deiner Gebiete (max. 3 oder Gebiete÷3)\n• Vollständige Kontinente geben Bonus-Truppen\n• Wähle Gebiet → Anzahl → Bestätigen",
        "easy_hint": "💡 Tipp: Stärke schwache Grenzen ODER sammle Truppen für einen großen Angriff!"
    },
    "attack": {
        "title": "⚔ ANGRIFF",
        "text": "1. Wähle eigenes Gebiet mit ≥2 Truppen\n2. Klicke auf feindliches Nachbargebiet\n3. Würfel entscheiden: Angreifer (bis 3) vs Verteidiger (bis 2)\n• Bei Gleichstand gewinnt Verteidiger",
        "easy_hint": "💡 Tipp: Greife nur an, wenn du deutlich mehr Truppen hast! 3:2 ist riskant, 4:2 ist sicherer."
    },
    "fortify": {
        "title": "🔀 VERSCHIEBEN",
        "text": "Bewege Truppen zwischen verbundenen eigenen Gebieten.\n• Wähle Startgebiet (≥2 Truppen)\n• Wähle Zielgebiet (muss erreichbar sein)\n• Verschiebe beliebige Anzahl (mind. 1 bleibt zurück)",
        "easy_hint": "💡 Tipp: Konzentriere Truppen an strategischen Grenzen – z.B. um einen Kontinent zu halten!"
    },
    "cards": {
        "title": "🃏 KARTEN-SYSTEM",
        "text": "• Bei jeder Eroberung: 1 Karte erhalten\n• 3 gleiche Karten ODER je 1 von jeder Art tauschen\n• Tausch-Bonus steigt: 4→6→8→10→12→15 Truppen\n• Max. 5 Karten halten – dann MUSS getauscht werden",
        "easy_hint": "💡 Tipp: Tausche früh, wenn der Bonus noch niedrig ist – oder warte auf große Offensive!"
    }
}

STRATEGIC_TIPS = [
    "🌏 Halte Australien (nur 2 Eingänge) für sicheren +2 Bonus!",
    "🗺️ Nordamerika ist stark, aber schwer zu verteidigen – fokussiere dich!",
    "⚠️ Grenzgebiete mit nur 1 Truppe sind verwundbar – verstärke sie!",
    "🃏 3 Karten sammeln = kostenlose Truppen – plane voraus!",
    "🎯 Eroberung von Südeuropa öffnet Zugang zu 3 Kontinenten!",
    "🛡️ Nach erfolgreichem Angriff: Truppen nachziehen nicht vergessen!",
    "📊 Statistik-Button zeigt deine Erfolgsquote – lerne daraus!",
]
...
class SetupDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        self.help_mode_var = tk.StringVar(value="easy")

        def spin(parent, var, from_, to):
            return tk.Spinbox(parent, from_=from_, to=to, textvariable=var, width=5,
        ...
        tk.Label(frame, text="Hilfe-Level:", fg="#c0c0e0", bg="#16213e", anchor="w", width=22).grid(row=3, column=0, sticky="w", pady=4)
        help_frame = tk.Frame(frame, bg="#16213e")
        help_frame.grid(row=3, column=1, sticky="w", padx=(8,0))
        for mode, lbl in [("easy", "🟢 Leicht (mit Tipps)"), ("medium", "🟡 Mittel (Basis)"), ("hard", "🔴 Schwer (keine Hilfe)")]:
            tk.Radiobutton(help_frame, text=lbl, variable=self.help_mode_var, value=mode,
                           fg="#c0c0e0", bg="#16213e", selectcolor="#0f3460",
                           activebackground="#16213e", activeforeground="white", anchor="w").pack(fill="x")
    ...
    def _update_name_fields(self):
        ...
        for i in range(n):
            ...
            tk.Label(f, text="●", fg=PLAYER_COLORS[i], bg="#1a1a2e", font=("Helvetica",14)).pack(side="left", padx=(0,6))
            v = tk.StringVar(value=f"Spieler {i+1}")
            tk.Entry(f, textvariable=v, bg="#0f3460", fg="white", insertbackground="white",
                     relief="flat", font=("Helvetica",11), width=20).pack(side="left")
    ...
    def _ok(self):
        ...
        self.result = {
            ...
            "help_mode": self.help_mode_var.get(),
        }
        self.destroy()
...
class RisikoApp(tk.Tk):
    def __init__(self):
        ...
        self.selected_from: Optional[str] = None
    ...
    def _build_ui(self):
        ...
        # FIX: Keine doppelten bg/fg Parameter mehr
        self.btn_help   = tk.Button(self.topbar, text="❓ Hilfe", command=self._show_help,
        ...
        self.hint_label = tk.Label(self.action_frame, text="", font=("Helvetica",9,"italic"),
                                   fg="#f39c12", bg="#16213e", wraplength=220, justify="left")
    ...
    def _init_tooltip(self):
        ...
        self.tooltip.configure(bg="#1a1a3e", relief="solid", borderwidth=1)
        self.tooltip_label = tk.Label(self.tooltip, text="", bg="#1a1a3e", fg="#c0c0e0", 
                                     font=("Helvetica",9), justify="left", padx=8, pady=4,
                                     wraplength=200)
        self.tooltip_label.pack()

    def _show_tooltip(self, text, x, y):
        if not HELP_MODES.get(self.help_mode, {}).get("tooltips", False):
            return
    ...
    def _on_canvas_motion(self, event):
        if not self.gs or not HELP_MODES.get(self.help_mode, {}).get("tooltips", False):
            return
        terr = self._territory_at(event.x, event.y)
        if terr and terr in TERRITORIES:
            data = self.gs.board[terr]
            owner = data["owner"]
            troops = data["troops"]
            cont = TERRITORIES[terr]["continent"]
            neighbors = len(TERRITORIES[terr]["neighbors"])
            
            tooltip_text = f"📍 {terr}\n"
            tooltip_text += f"👤 {owner or 'Niemand'}\n"
            tooltip_text += f"⚔ {troops} Truppen\n"
            tooltip_text += f"🌍 {cont} | 🔗 {neighbors} Nachbarn"
            
            if owner and self.gs.phase == "attack" and not self.gs.current_player().is_ai:
                if owner == self.gs.current_player().name and troops >= 2:
                    tooltip_text += "\n\n✅ Kann angreifen"
                elif owner != self.gs.current_player().name:
    ...
    # ── Reset & Clean Functions ──
    def _clear_log(self):
        """Löscht das Ereignis-Log komplett"""
        self.log_text.configure(state="normal")
        self.log_text.delete("1.0", "end")
        self.log_text.configure(state="disabled")

    def _reset_timers_and_flags(self):
        """Bricht laufende Timer ab und setzt Flags zurück"""
        if self._hint_timer:
    ...
    def _new_game(self):
        ...
        self._reset_timers_and_flags()
        self._clear_log() # FIX: Log wird bei neuem Spiel geleert
        
        gs = GameState()
        gs.help_mode = cfg["help_mode"]
        self.help_mode = cfg["help_mode"]
        color_idx = 0
        ...
        if HELP_MODES.get(self.help_mode, {}).get("tutorial", False) and not self._tutorial_active:
            self._tutorial_active = True
            self._tutorial_step = 0
            self.after(1000, self._show_tutorial_overlay)
    ...
    def _run_initial_placement(self):
        ...
        if player.is_ai:
            ...
            help_info = HELP_TEXTS.get("initial", {})
            hint = help_info.get("easy_hint", "") if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{help_info.get('title', '')}\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
            self._set_action(msg, show_confirm=False)
    ...
    def _start_turn(self):
        ...
        if player.is_ai:
            ...
            help_info = HELP_TEXTS.get("reinforcement", {})
            hint = help_info.get("easy_hint", "") if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{help_info.get('title', '')}\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
            self._set_action(msg, show_spin=True, spin_max=n)

    def _show_strategic_hint(self):
        if not HELP_MODES.get(self.help_mode, {}).get("strategic_tips", False):
            self.hint_label.configure(text="")
            return
        if not self.gs or self.gs.current_player().is_ai:
            self.hint_label.configure(text="")
            return
            
        tip = random.choice(STRATEGIC_TIPS)
        self.hint_label.configure(text=tip)
        
        if self._hint_timer:
            self.after_cancel(self._hint_timer)
        self._hint_timer = self.after(30000, self._show_strategic_hint)
    ...
    def _next_phase(self):
        ...
        if self.gs.phase == "reinforcement":
            ...
            self.selected_from = None
            self._set_phase_label()
            help_info = HELP_TEXTS.get("attack", {})
            hint = help_info.get("easy_hint", "") if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{help_info.get('title', '')}\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
            ...
            self.selected_from = None
            self._set_phase_label()
            help_info = HELP_TEXTS.get("fortify", {})
            hint = help_info.get("easy_hint", "") if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{help_info.get('title', '')}\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
    ...
    def _handle_reinforce_click(self, terr, player):
        ...
        if self.gs.pending_troops > 0:
            ...
            help_info = HELP_TEXTS.get("reinforcement", {})
            hint = help_info.get("easy_hint", "") if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{self.gs.pending_troops} Truppe(n) verbleibend.\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
            self._set_action(msg, show_spin=True, spin_max=max_v)
        else:
            self._set_action("✅ Alle Truppen platziert!\n\n▶ Klicke 'Nächste Phase' für Angriff", show_confirm=False)
    ...
    def _handle_attack_click(self, terr, player):
        ...
        if self.selected_from is None:
            if board[terr]["owner"] == player.name and board[terr]["troops"] >= 2:
                ...
                help_info = HELP_TEXTS.get("attack", {})
                msg = f"🎯 Von: {terr} ({board[terr]['troops']} Tr.)\n\n{help_info.get('text', '').split('2.')[1].strip() if '2.' in help_info.get('text', '') else '→ Klicke auf feindliches Nachbargebiet.'}"
                self._set_action(msg, show_confirm=False)
                self._redraw_map()
            else:
                help_info = HELP_TEXTS.get("attack", {})
                msg = f"⚠️ Wähle eigenes Gebiet mit ≥2 Truppen.\n\n{help_info.get('text', '')}"
                self._set_action(msg, show_confirm=False)
        else:
            if terr == self.selected_from:
                self.selected_from = None
                help_info = HELP_TEXTS.get("attack", {})
                self._set_action(f"Auswahl aufgehoben.\n\n{help_info.get('text', '')}", show_confirm=False)
                self._redraw_map()
                return
            neighbors = TERRITORIES[self.selected_from]["neighbors"]
            if terr not in neighbors or board[terr]["owner"] == player.name:
                self.selected_from = None
                self._set_action("❌ Kein gültiges Ziel.\nBitte neu wählen.", show_confirm=False)
                self._redraw_map()
                return
    ...
    def _do_attack(self, from_t, to_t, player):
        ...
        result = f"⚔ {from_t} → {to_t}\n  Angreifer: [{', '.join(str(r) for r in atk_r)}] vs Verteidiger: [{', '.join(str(r) for r in def_r)}]"
        if conquered:
            result += f"\n  ✅ {to_t} eingenommen!"
            ...
            result += f"\n  Ergebnis: Angreifer -{def_w}, Verteidiger -{atk_w}"
        self._log(result)
        self._redraw_map()
        self._update_player_status()
        self.gs.check_winner()
        if self.gs.game_over:
            ...
            help_info = HELP_TEXTS.get("attack", {})
            hint = "💡 Tipp: Mehrere kleine Angriffe sind oft besser als ein großer!" if HELP_MODES.get(self.help_mode, {}).get("hints", False) else ""
            msg = f"{'✅ Angriff erfolgreich!' if conquered else '⚔ Angriff beendet.'}\n\n{help_info.get('text', '')}"
            if hint:
                msg += f"\n\n{hint}"
            self._set_action(msg, show_confirm=False)
    ...
    def _handle_fortify_click(self, terr, player):
        ...
        if self.selected_from is None:
            ...
            if terr not in reachable:
                self._set_action("❌ Nicht verbunden!\nWähle ein erreichbares Gebiet.", show_confirm=False)
                self.selected_from = None
                self._redraw_map()
                return
            ...
            self._redraw_map()
            help_info = HELP_TEXTS.get("fortify", {})
            self._set_action(f"✅ Verschoben!\n\n{help_info.get('text', '')}\n\n▶ Klicke 'Nächste Phase' um Zug zu beenden", show_confirm=False)
    ...
    def _redraw_map(self):
        ...
        for t, data in TERRITORIES.items():
            ...
            show_highlights = HELP_MODES.get(self.help_mode, {}).get("highlight_moves", False)
            is_recommended = False
            if show_highlights and self.gs.phase == "reinforcement" and not self.gs.current_player().is_ai:
                if owner_name == self.gs.current_player().name:
                    is_border = any(self.gs.board[nb]["owner"] != player.name for nb in TERRITORIES[t]["neighbors"])
                    if is_border and troops <= 2:
                        is_recommended = True

            r = max(14, int(16 * min(sx, sy)))
            outline_color = "#ffffff" if is_selected else ("#ff4444" if is_attackable else ("#44aaff" if is_fortifiable else ("#2ecc71" if is_recommended else cont_color)))
            outline_w = 4 if (is_selected or is_attackable or is_fortifiable or is_recommended) else 1
            ...
            if HELP_MODES.get(self.help_mode, {}).get("strategic_tips", False):
                cont = data["continent"]
                if CONTINENTS[cont]["bonus"] >= 5:
                    c.create_text(x+12, y-12, text=f"+{CONTINENTS[cont]['bonus']}", 
                                 fill="#f39c12", font=("Helvetica", 7, "bold"), tags=("territory", t))
    ...
    def _show_help(self):
        ...
        win.title("❓ Hilfe & Tipps")
        win.configure(bg="#0d1a2e")
        win.geometry("500x450")
        win.grab_set()
        
        tab_frame = tk.Frame(win, bg="#0d1a2e")
        tab_frame.pack(fill="x", padx=8, pady=(8,0))
        
        notebook = ttk.Notebook(tab_frame)
        notebook.pack(fill="both", expand=True, padx=4, pady=4)
        
        phase_tab = tk.Frame(notebook, bg="#0d1a2e")
        notebook.add(phase_tab, text="📋 Aktuell")
        
        phase = self.gs.phase if not self.initial_mode else "initial"
        help_info = HELP_TEXTS.get(phase, {})
        
        tk.Label(phase_tab, text=help_info.get("title", "Hilfe"), font=("Helvetica",14,"bold"),
                 fg="#f0c040", bg="#0d1a2e").pack(pady=12)
        tk.Label(phase_tab, text=help_info.get("text", ""), font=("Helvetica",10),
                 fg="#c0d0e0", bg="#0d1a2e", wraplength=450, justify="left").pack(pady=8)
        
        if HELP_MODES.get(self.help_mode, {}).get("hints", False) and help_info.get("easy_hint"):
            tk.Label(phase_tab, text=help_info["easy_hint"], font=("Helvetica",10,"italic"),
                     fg="#f39c12", bg="#0d1a2e", wraplength=450, justify="left").pack(pady=8)
        
        cards_tab = tk.Frame(notebook, bg="#0d1a2e")
        notebook.add(cards_tab, text="🃏 Karten")
        tk.Label(cards_tab, text=HELP_TEXTS["cards"]["title"], font=("Helvetica",14,"bold"),
                 fg="#f0c040", bg="#0d1a2e").pack(pady=12)
        tk.Label(cards_tab, text=HELP_TEXTS["cards"]["text"], font=("Helvetica",10),
                 fg="#c0d0e0", bg="#0d1a2e", wraplength=450, justify="left").pack(pady=8)
        if HELP_MODES.get(self.help_mode, {}).get("hints", False):
            tk.Label(cards_tab, text=HELP_TEXTS["cards"]["easy_hint"], font=("Helvetica",10,"italic"),
                     fg="#f39c12", bg="#0d1a2e", wraplength=450, justify="left").pack(pady=8)
        
        cont_tab = tk.Frame(notebook, bg="#0d1a2e")
        notebook.add(cont_tab, text="🌍 Kontinente")
        for cont, data in CONTINENTS.items():
            row = tk.Frame(cont_tab, bg="#0d1a2e")
            row.pack(fill="x", pady=2)
            tk.Label(row, text="■", fg=data["color"], bg="#0d1a2e", font=("Helvetica",12)).pack(side="left", padx=4)
            tk.Label(row, text=f"{cont}: +{data['bonus']} Truppen/Runde", 
                     fg="#c0d0e0", bg="#0d1a2e", font=("Helvetica",10)).pack(side="left")
        
        settings_tab = tk.Frame(notebook, bg="#0d1a2e")
        notebook.add(settings_tab, text="⚙️ Einstellungen")
        
        tk.Label(settings_tab, text="Hilfe-Level ändern:", fg="#c0c0e0", bg="#0d1a2e",
                 font=("Helvetica",10)).pack(pady=(12,4))
        
        help_var = tk.StringVar(value=self.help_mode)
        for mode, lbl in [("easy", "🟢 Leicht (mit Tipps)"), ("medium", "🟡 Mittel (Basis)"), ("hard", "🔴 Schwer (keine Hilfe)")]:
            tk.Radiobutton(settings_tab, text=lbl, variable=help_var, value=mode,
                           fg="#c0c0e0", bg="#0d1a2e", selectcolor="#0f3460",
                           activebackground="#0d1a2e", activeforeground="white",
                           command=lambda m=mode: self._change_help_mode(m)).pack(anchor="w", padx=20)
        
        tk.Button(win, text="Schließen", command=win.destroy,
                  bg="#333355", fg="white", relief="flat", padx=20, pady=6).pack(pady=12)

    def _change_help_mode(self, mode):
        self.help_mode = mode
        if self.gs:
            self.gs.help_mode = mode
        self._log(f"⚙️ Hilfe-Level geändert: {mode}")
        if mode == "easy":
    ...
    # ── Tutorial Overlay ──
    def _show_tutorial_overlay(self, step=None):
        if not HELP_MODES.get(self.help_mode, {}).get("tutorial", False):
            return
            
        tutorial_steps = [
            {
                "title": "🎮 Willkommen zu RISIKO!",
                "text": "Erobere die Welt! Jeder Spieler erhält abwechselnd:\n• Verstärkungstruppen\n• Angriffsphase\n• Verschiebe-Phase\n\nZiel: Alle Gegner eliminieren! 🏆",
                "action": None
            },
            {
                "title": "📍 Startaufstellung",
                "text": "Platziere deine Starttruppen:\n• Klicke auf DEINE Gebiete\n• Jeder Klick = +1 Truppe\n• Alle Spieler machen das abwechselnd\n\n💡 Tipp: Grenzgebiete zuerst stärken!",
                "action": "initial"
            },
            {
                "title": "➕ Verstärkung",
                "text": "Du erhältst Truppen basierend auf:\n• Anzahl deiner Gebiete\n• Vollständige Kontinente = Bonus!\n\nWähle Gebiet → Anzahl → Bestätigen\n\n💡 Tipp: Schwache Grenzen priorisieren!",
                "action": "reinforcement"
            },
            {
                "title": "⚔ Angriff",
                "text": "1. Eigenes Gebiet wählen (≥2 Truppen)\n2. Feindliches Nachbargebiet anklicken\n3. Würfel entscheiden!\n\n💡 Tipp: Nur angreifen bei klarem Vorteil!",
                "action": "attack"
            },
            {
                "title": "🔀 Verschieben",
                "text": "Bewege Truppen zwischen eigenen Gebieten:\n• Wähle Start → Ziel\n• Mindestens 1 Truppe muss bleiben\n\n💡 Tipp: Truppen an strategische Grenzen!",
                "action": "fortify"
            },
            {
                "title": "🃏 Karten-System",
                "text": "Bei jeder Eroberung: 1 Karte!\n• 3 gleiche ODER je 1 von jeder Art tauschen\n• Bonus: 4→6→8→10→12→15 Truppen\n• Max. 5 Karten halten\n\n💡 Tipp: Früh tauschen oder für große Offensive sparen!",
                "action": None
            },
            {
                "title": "✅ Tutorial abgeschlossen!",
                "text": "Du bist bereit für die Welteroberung! 🌍⚔\n\n💡 Nutze den ❓ Hilfe-Button jederzeit für Tipps.\n\nViel Erfolg, Kommandant! 🏆",
                "action": "complete"
            }
        ]
        ...
        overlay.title("🎓 Tutorial")
        overlay.configure(bg="#0d1a2e")
        overlay.geometry("420x380")
        overlay.grab_set()
        overlay.transient(self)
        
        tk.Label(overlay, text=data["title"], font=("Georgia",16,"bold"),
                 fg="#f0c040", bg="#0d1a2e", wraplength=380).pack(pady=(20,8))
        tk.Label(overlay, text=data["text"], font=("Helvetica",10),
                 fg="#c0d0e0", bg="#0d1a2e", wraplength=380, justify="left").pack(pady=8)
        
        progress = tk.Frame(overlay, bg="#0d1a2e")
        progress.pack(pady=12)
        for i in range(len(tutorial_steps)):
            color = "#2ecc71" if i < step else ("#f0c040" if i == step else "#404040")
            tk.Label(progress, text="●", fg=color, bg="#0d1a2e", font=("Helvetica",12)).pack(side="left", padx=2)
        
        btn_frame = tk.Frame(overlay, bg="#0d1a2e")
        btn_frame.pack(pady=16)
        ...
        def next_step():
            ...
            if self._tutorial_step < len(tutorial_steps):
                self.after(300, lambda: self._show_tutorial_overlay(self._tutorial_step))
            else:
                self._tutorial_active = False
                self._log("✅ Tutorial abgeschlossen! Viel Erfolg! 🏆")
                if HELP_MODES.get(self.help_mode, {}).get("strategic_tips", False):
                    self._show_strategic_hint()
        
        btn_text = "Los geht's! 🚀" if step == len(tutorial_steps) - 1 else "Weiter ▶"
        tk.Button(btn_frame, text=btn_text, command=next_step,
                  bg="#2ecc71", fg="white", font=("Helvetica",12,"bold"),
                  relief="flat", padx=24, pady=10, cursor="hand2").pack()
                  
        if step < len(tutorial_steps) - 1:
            tk.Button(overlay, text="Tutorial überspringen", command=lambda: (overlay.destroy(), setattr(self, '_tutorial_active', False)),
                      bg="#333355", fg="#808090", font=("Helvetica",9),
                      relief="flat", padx=12, pady=4, cursor="hand2").pack(pady=(0,8))
            ...
            if sets:
                ...
                if messagebox.askyesno("Karten", msg + "\n\nJetzt tauschen?", parent=self):
                    ...
                    if self.gs.phase == "reinforcement":
                        ...
                        help_info = HELP_TEXTS.get("reinforcement", {})
                        self._set_action(f"{n} Truppe(n) zu platzieren.\n\n{help_info.get('text', '')}", show_spin=True, spin_max=n)
                return
    ...
    def _show_stats(self):
        ...
        for j, col in enumerate(cols):
            tk.Label(f, text=col, fg="#606080", bg="#0d1a2e",
                     font=("Helvetica",9,"bold"), width=11).grid(row=0, column=j, pady=4)
        ...
        for i, p in enumerate(self.gs.players):
            ...
            for j, v in enumerate(vals):
                ...
                tk.Label(f, text=str(v), fg=fg, bg="#0d1a2e",
                         font=("Helvetica",10), width=11).grid(row=i+1, column=j, pady=3)
    ...
    def _show_rules(self):
        ...
        tk.Label(win, text="RISIKO – SPIELREGELN", font=("Georgia",14,"bold"),
                 fg="#f0c040", bg="#0d1a2e").pack(pady=12)
        ...
        rules = """ZIEL: Alle Gebiete der Welt erobern und Gegner eliminieren.

🔄 SPIELABLAUF je Runde:
  1️⃣ VERSTÄRKUNG: 
     • Truppen = max(3, eigene_Gebiete÷3) + Kontinent-Boni
     • Auf eigene Gebiete platzieren
     
  2️⃣ ANGRIFF (optional):
     • Eigenes Gebiet (≥2 Tr.) → feindliches Nachbargebiet
     • Angreifer: bis 3 Würfel | Verteidiger: bis 2 Würfel
     • Bei Gleichstand gewinnt VERTEIDIGER
     • Bei Eroberung: Truppen nachziehen + ggf. Karte erhalten
     
  3️⃣ VERSCHIEBEN (optional):
     • Truppen zwischen verbundenen eigenen Gebieten bewegen
     • Mindestens 1 Truppe muss im Startgebiet bleiben

🌍 KONTINENT-BONI (pro Runde):
  • Nordamerika: +5    • Südamerika: +2
  • Europa: +5         • Afrika: +3  
  • Asien: +7          • Australien: +2

🃏 KARTEN-SYSTEM:
  • +1 Karte bei jeder Eroberung
  • Tauschen: 3 gleiche ODER je 1 von jeder Art
  • Bonus steigt: 4→6→8→10→12→15 Truppen
  • Bei 5 Karten: MUSS getauscht werden

💡 STRATEGIE-TIPPS:
  • Australien ist einfach zu halten (nur 2 Eingänge)
  • Grenzgebiete mit 1 Truppe sind verwundbar
  • Karten früh tauschen = viele kleine Boni
  • Karten spät tauschen = großer Bonus für Finale

❓ HILFE-FUNKTIONEN:
  • Maus über Gebiet = Tooltip mit Infos
  • ❓ Button = kontextsensitive Hilfe
  • 🟢 Easy Mode: Tipps, Highlights, Tutorial
  • 🟡 Medium: Basis-Hilfe
  • 🔴 Hard: Keine Hilfen (Puristen-Modus)
"""
